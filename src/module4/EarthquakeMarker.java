...
public abstract class EarthquakeMarker extends SimplePointMarker
{
	...
	private void colorDetermine(PGraphics pg) {
		...
		// Shallow earthquakes are red
		if (depth < THRESHOLD_INTERMEDIATE) {
			pg.fill(255, 0, 0);
		}
		
		// Intermediate earthquakes are blue
		if ((depth > THRESHOLD_INTERMEDIATE) && (depth < THRESHOLD_DEEP)){
			pg.fill(0, 0, 255);
		}
		
		// Deep earthquakes are green
		if (depth > THRESHOLD_DEEP) {
			pg.fill(0, 255, 0);
		}
	}
	
	// determine if the marker will have an X over it based on the age of the quake
