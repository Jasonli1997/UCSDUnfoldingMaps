...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		for (Marker cm : countryMarkers) {
			int count = 0;
			String name = cm.getStringProperty("name");
			for (Marker earthquake : quakeMarkers) {
				
				// Cast earthquake in EarthquakeMarker to access the isOnLand method
				EarthquakeMarker em = (EarthquakeMarker) earthquake;
				
				if (em.isOnLand) {
					String country = earthquake.getStringProperty("country");
			...
			// Only prints countries with one or more earthquakes
			if (count >= 1) {
